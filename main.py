...
def showrecommendedusers():
    ...
    return render_template(
        "showrecommendedusers.html", User=User, Recommendedusers=Recommendedusers
    )
...
def showprofile(Userid):
    ...
    if int(Userid) == session["logged_in"]["Userid"]:
        return redirect(url_for("showmyprofile"))
    if not dbmediator.isvaliduserid(Userid).data:
        return redirect(url_for("main"))
    ...
    return render_template(
        "showprofile.html", User=User, Otheruser=Otheruser, isfollowing=isfollowing
    )
...
def signup():
    ...
    if request.method == "POST":
        ...
        if ret.success:
            ...
            send_verification_email(Userid)
            command = (
                "cp static/default-profile-pic.jpg static/pp" + str(Userid) + ".jpg"
            )
            os.system(command)
...
def evaluate_submission(Submissionid, Questionid, Userid):
    ...
    if process.returncode != 0:
        ...
        testcasesinput = "".join(
            readlines("questions/q" + str(Questionid), "/Testcaseinput.txt")
        )
        try:
            process = subprocess.run(
                ["usersubmissions/" + str(Submissionid) + "/Compiled.exe"],
                text=True,
            ...
            if process.returncode != 0:
                ...
                correctoutput = "".join(
                    readlines(
                        "questions/q" + str(Questionid) + "/", "Testcaseoutput.txt"
                    )
                )
                useroutput = stdout
                f = open(
                    "usersubmissions/" + str(Submissionid) + "/Useroutput.txt", "w+"
                )
                f.write(stdout)
                f.close()
                if correctoutput == useroutput:
                    ...
                    Maxkudos = dbmediator.getmaxkudosgotinthisquestion(
                        Questionid, Userid
                    ).data
                else:
                    Remarks = "WA"
...
def addquestion():
    ...
    if request.method == "POST":
        ...
        flash(
            "Question added successfully! Please wait for Admin's approval for getting Kudos!"
        )
...
def approveordeclinequestion(Questionid):
    ...
    if request.method == "POST":
        if request.form["decisionradio"] == "approved":
            dbmediator.approvequestion(
                Questionid, request.form["Maxkudos"], request.form["difficultylevel"]
            )
    ...
    Question = loadquestion(Questionid)
    return render_template(
        "approveordeclinequestion.html", Question=Question, User=User
    )
...
def downloadtestcaseinput(Questionid):
    ...
    return send_from_directory(
        "questions/q" + str(Questionid), "Testcaseinput.txt", as_attachment=True
    )
...
def downloadtestcaseoutput(Questionid):
    ...
    if not dbmediator.isadmin(session["logged_in"]["Userid"]).data:
        return redirect(url_for("main"))
    return send_from_directory(
        "questions/q" + str(Questionid), "Testcaseoutput.txt", as_attachment=True
    )
...
def downloadsubmittedcode(Submissionid):
    if not check_session():
        return redirect(url_for("login"))
    if (
        not dbmediator.isadmin(session["logged_in"]["Userid"]).data
        and dbmediator.getwhosubmittedcode(Submissionid).data
        != session["logged_in"]["Userid"]
    ):
        return redirect(url_for("main"))
    return send_from_directory(
        "usersubmissions/" + str(Submissionid), "Solution.cpp", as_attachment=True
    )
